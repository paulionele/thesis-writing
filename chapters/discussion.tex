...
\section{Preparation for Analysis}
	Since the same tools will be used to analyze similar plots for the 1D and 2D systems, this section serves to outline some fundamentals that will be referred to and utilized frequently in the analysis.
	...
	This factor $ \beta $ can be used to determine and quantify the diffusion regime of the system. Taking the base-10 logarithm of both sides of Equation \ref{eq:einstein-modified} gives:
	...
	A plot of $ \beta (t) $ versus $ t $ can reveal the magnitude of the non-linearity and identify the diffusion regime (sub- or super-diffusive) the entire system is in at a particular time. For a sub-diffusive process $ \beta < 1 $, for a super-diffusive process $ \beta > 1 $, and for steady diffusion $ \beta = 1 $.
	...
	Plots of $ D_\textrm{eff} $ versus $ t $ may be constructed and are useful in determining the long-time effective diffusion and comparing 
...
\label{sec:1D-boundary-transition-probability}
	The total transition probability is the probability that a particle steps towards a semi-permeable boundary and then crosses that boundary. 
	
	 Referring to Figure \ref{fig:pie_msd_1D}, the slope is clearly not unity for some of the plots.
